...
> **2026-06-27 更新（入口の取り逃し是正 = 浅い押し目）**:
>
> - **買いの提案指値の既定を `ma`（5日線の近辺）→ `atr`（0.25·ATR の浅い押し目）に変更**。
>   verify-before-adopt 診断（実データ3年・5銘柄・出口R:R固定）で、5日線への深い押し目待ちは
>   伸びる玉を取り逃し約定率 36.8% に留まると判明。0.25·ATR の浅い押し目で **約定率 37→67%・
>   OOS リターン 8.8→16.5%・ドローダウン改善**（過学習なし）。深さは `entry_atr_mult` で設定可変。
>   既存DBは旧既定（ma/0.5）の行だけ非クロバーで一度だけ移行。出口の R:R 4:1（利確 6·ATR／
>   損切 1.5·ATR）の上に積む是正。
>
