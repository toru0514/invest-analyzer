...
> - ダッシュボードに「出来高倍率」「週足」列を追加。設定 UI で上記 3 フィルターの params も編集可能。
>
...
- **出口入りシミュレーション（追補版 強化J）は未実装**。バックテストはスコア反転で決済する方式で、
  ATR の損切/利確ラインに当日高安が触れたら決済する方式（＝出口設計の良し悪しの検証）は未対応。
- **週足は日足からリサンプルして算出**（追補版の「週足を別 fetch・`timeframe` 列で保持」ではなく、
  同一日足データから週足化）。look-ahead を避けやすく追加の取得も不要なため。挙動（週足 SMA13 の傾き）は同じ。
- **乖離率など追加指標は未着手**（現状は 6 指標 + 出来高/週足/ATR フィルター + 指定金額アラート）。
- **frontend の自動テストは未整備**（backend は API 結合 + スケジューラのテストあり。UI は手動/E2E 目視）。
...
2. **指標の拡充と検証**：出来高系・ATR・乖離率などを追加し、バックテストで取捨選択。
