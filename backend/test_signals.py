...
import signals
import phase0_backtest as bt


def test_evaluate_returns_valid_direction():
    df = bt.synthetic_history("TEST.T", n=90, seed=42)
...
def test_golden_dead_cross_are_exclusive():
    df = bt.synthetic_history("TEST.T", n=90, seed=1)
    g = signals.golden_cross(df, 5, 25)
    d = signals.dead_cross(df, 5, 25)
    assert not (g and d)  # 同日に両方は成立しない


def test_backtest_reports_all_required_metrics():
    hist = {tk: bt.synthetic_history(tk, seed=i)
            for i, tk in enumerate(["8306.T", "7203.T", "9984.T"])}
    r = bt.run_backtest(hist)
    for key in ("initial", "final", "pnl_amount", "pnl_pct",
                "trade_count", "win_rate", "max_drawdown_pct"):
        assert key in r
    assert r["initial"] == bt.INITIAL_CAPITAL
    assert r["trade_count"] >= 0
    assert r["max_drawdown_pct"] >= 0


def test_trades_execute_when_thresholds_low(monkeypatch=None):
    # 閾値を下げれば必ず取引が発生し、決済損益・成績が集計されることを確認
