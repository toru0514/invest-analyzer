...
- `/optimize` の leave-one-out 寄与度：指標ルールを1つ外すとそのグループの raw 合計→clip が変わるため、**グループ化後の寄与**として引き続き機能する。
  - 注：逆張り群のように raw が cap を超えて飽和している場合、メンバーを1つ外しても clip 後が変わらず **delta≈0** になり得る（グループ化後は個別寄与の感度が下がる）。`/optimize` 寄与度の解釈時に留意。
- **スコアのレンジ**：グループ化後の最大は **±4**（4グループ×±1）。既定閾値 ±2 は「2グループ一致」で到達＝妥当。`/optimize` のスイープ `[2,3,4]`（`evaluation.py`）は max=4 に対し有効範囲を張る（4＝全グループ一致の最厳）。
...
- `test_signals.py` の**既存スコア等値アサーションは単一指標構成（`disparity`/`obv`/`cci` の `score == 1`）が中心で、単独指標はグループ内 clip でも ±1 のまま＝そのまま通る見込み**。多重カウント解消は**複数指標同時発火**で現れる。よって「壊れたテストの修正」ではなく**新規のグループ化テストを追加**し、既存は**実行して実際に壊れたものだけ最小更新**する（先回り編集はしない）。direction の妥当性のみ見るテストは不変。
...
- **ローソク足のグループclip**：赤三兵＋包み足が同方向に同時発火（従来 +2 になり得た）でも `pattern` グループは **±1 に収まる**こと。逆向き同時発火が 0 に相殺される挙動も確認（意図的な仕様）。
