...
```
relative_strength(ticker_df, index_df, n, scale, asof=None) → s_rs ∈ [-1, 1]   # 新規ヘルパ
  └ asof（評価日）以前に双方を絞って look-ahead を担保（_regime_at の .asof(d) と同じく過去のみ参照）
...
- `evaluate_holdout(..., regime_series=None, index_history=None, rs_params=None)` を追加し、内部クロージャ `_bt`（L94-98）の全 `run_backtest`（train/leave-one-out/oos）**および内部の `benchmark` 呼び出し（`evaluation.py:130`）**へ素通し（`regime_series` がこの nested benchmark にも渡っているのと同じ。OOS ベンチ側だけ RS 無効になり比較条件がズレるのを防ぐ）。
...
- `backend/signals.py`:
  - `relative_strength(ticker_df, index_df, n, scale, asof=None)` ヘルパ新規。
