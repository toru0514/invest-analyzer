...
def test_daily_plan_sizing_columns_and_upsert(tmp_path, monkeypatch):
    ...
    # get_conn() は呼び出し時にモジュール global db.DB_PATH を読む（db.py:123 は @contextmanager）。
...
> 注（DB 差し替えの仕組み・plan-reviewer 指摘の確定事項）: 実 env 変数は **`INVEST_DB_PATH`**（`DB_PATH` ではない・`db.py:15`）で、これがモジュール global `db.DB_PATH` に入る。`test_api.py` は import 時に `INVEST_DB_PATH` を一時 DB に向けて全体を隔離済み。本テストはさらに `monkeypatch.setattr(db, "DB_PATH", ...)` で局所的に差し替える（`get_conn()` は呼び出し時に `DB_PATH` を読むため有効・teardown で自動復元）。`importlib.reload` は `main` が抱える `db` 参照を汚染し後続テスト（Task 4 の `client` ベーステスト等）を壊すので**使わない**。`get_conn()` は必ず `with ... as conn:` で使う。
...
Run: `npm --prefix frontend test -- rows`（`test` スクリプトは既に `vitest run`。`rows` はファイル名フィルタ）
...
Run: `npm --prefix frontend test -- rows` → PASS。続けて `npm --prefix frontend run build`（型エラーが無いこと）。
...
Run: `npm --prefix frontend test` → 13 + 2（riskSummary）件 PASS
