...
def evaluate_holdout(histories, configs, *, split_ratio=0.7, grid=None, cost=None,
    ...
    def _bt(hist, th, eval_start=None):
        return run_backtest(hist, configs=configs, initial_capital=initial_capital,
    ...
    for th in grid["threshold"]:
        ...
        if best is None or key > best[0]:
            best = (key, th, r, stat)
    _, best_th, train_r, train_stat = best

    # out-of-sample：選んだ閾値で全履歴を使い、約定は split 以降のみ
    oos_r = _bt(histories, best_th, eval_start=split)
    ...
    return {
        ...
        "in_sample": {"sample": "in_sample", "sweep": sweep, "pnl_pct": train_r["pnl_pct"],
                      "expectancy": train_stat["expectancy"], "trade_count": train_r["trade_count"],
...
> 注：旧 `sweep`/`contributions`/`best`/`baseline_pnl_pct` は `evaluate_holdout` の `in_sample` 配下（`sweep`）に集約済み。contributions（leave-one-out）は本タスクでは省略（YAGNI・将来 in_sample 配下に追加可能）。フロントが旧キーに依存していれば別途追従が要るが、本計画のスコープ外（必要なら追補）。
